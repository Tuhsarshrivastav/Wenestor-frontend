...
import BusinessSecondRow from "./BusinessSecondRow";
import { BUSINESS } from "../../../constants";
import { logout } from "../../../redux/actions/loginAction";
import LoadModal from "../../common/LoadModal";
import { loadOptions } from "@babel/core";
function BusinessDashboardOld({
  projects,
  loadProjects,
  deleteProject,
  logout,
  ...
  useEffect(() => {
    ...
    loadProjects()
      .then((res) => console.log(res), setApi(false))
      .catch((err) => {
        {
          console.log(err);
  ...
  //Edit Project
  const onEdit = (id) => {
    props.history.push("business/post/" + id);
  };
  //More click
  ...
  const onCardClick = (id) => {
    console.log("on Card click id ==> ", id);
    setSecondRow(true);
    setCurrProjectId(id);
        ...
        <div className="row">
          <div className="col-lg-6 col-sm-12">
            ...
            <div className="row center mb-3 center-align justify-content-center ">
              <Link className="btn btn-success add-btn" to="/business/post">
                + {"   "}Add
              </Link>
            </div>
          </div>
...
BusinessDashboardOld.propTypes = {
